...
class TestCollectorTypesMigration:

    def test_string_promoted_to_list(self):
        existing = {**SAMPLE_CONFIG, "collector_type": "braiins"}
        existing.pop("collector_types", None)
        result = _run_wizard(existing, _wizard_answers({}))
        assert result["collector_types"] == ["braiins"]

    def test_old_key_removed(self):
        existing = {**SAMPLE_CONFIG, "collector_type": "braiins"}
        result = _run_wizard(existing, _wizard_answers({}))
        assert "collector_type" not in result

    def test_existing_list_preserved(self):
        existing = {**SAMPLE_CONFIG, "collector_types": ["braiins", "luxos"]}
        existing.pop("collector_type", None)
        result = _run_wizard(
            existing,
            _wizard_answers({"Collector OS type(s)": "braiins, luxos"}),
        )
        assert result["collector_types"] == ["braiins", "luxos"]
        assert "collector_type" not in result

    def test_multi_os_selection(self):
        existing = {**SAMPLE_CONFIG}
        result = _run_wizard(
            existing,
            _wizard_answers({"Collector OS type(s)": "braiins, luxos, vnish"}),
        )
        assert result["collector_types"] == ["braiins", "luxos", "vnish"]

    def test_unknown_os_filtered_out(self):
        existing = {**SAMPLE_CONFIG}
        result = _run_wizard(
            existing,
            _wizard_answers({"Collector OS type(s)": "braiins, future-fw"}),
        )
        assert result["collector_types"] == ["braiins"]
    ...
    def test_all_unknown_falls_back_to_default(self):
        ...
        result = _run_wizard(
            existing,
            _wizard_answers({"Collector OS type(s)": "future-fw"}),
        )
        assert result["collector_types"] == ["braiins"]
